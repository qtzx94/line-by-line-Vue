...
描述：判断给定变量是否是未定义，当变量值为 null时，也会认为其是未定义。
...
描述：判断给定变量是否是定义，当变量值为 null时，也会认为其是未定义。
...
描述：判断给定变量是否是原始类型值，即：string、number、boolean以及 symbol。
...
源码分析： 当变量值为 null 时，返回空字符串；当值的类型为 object 返回 JSON.stringify(val, null, 2)，否则返回 String(val)。
...
首先，大家要明白这个函数的意义，我们提到了，传递给 cached 函数的参数一定要是一个纯函数，那为什么要是一个纯函数呢？因为纯函数有一个特性，即输入不变则输出不变。
在现实中，有很多这样的场景，简单举个例子，也是我们接下来要介绍的一个函数：中横线转驼峰 (camelize() 函数)，假设我们给 camelize 函数传递字符串 aaa-bbb，那么得到的始终都是 aaaBbb，不会有其他可能，
那我们想象一下，在一个庞大的应用程序中，我们可能需要转译很多相同的字符串，如果每次都要重新执行转译程序，那么是一个极大的浪费，我们只需转译一次并将结果缓存，
当再次需要转译该相同的字符串时，我们只需要从缓存中读取即可，这就是 cached 的目标，下面我们看一下它是怎么实现的。
...
这是一个很基本的函数，定义一个正则表达式：/-(\w)/g，用来全局匹配字符串中 中横线及连字符后的一个字符，注意该正则中拥有一个捕获组，用来捕获连字符后面的字符，
在 camelize 函数体内，使用 camelizeRE 正则匹配字符串，如果连字符后有字符，则将匹配到的内容使用该字符的大写形式替换，否则使用空字符串替换即可。
...
{any} list 类数组list
{number} start 开始转换索引
源码分析： toArray 接受2个参数，分别为类数组 list 和开始转换索引 start（默认从0开始）。通过new Array() 创建长度为 i 的新数组，while 循环对 ret 每一项赋值，最后返回转换后的新数组 ret。
...
looseEqual 以a、b两个变量为参数，返回 boolean 值。 当 a === b 时，返回true
